...
class DateDimension(object):
    def __init__(self, start: str, end: str):
        ...
        if not is_string_iso_8601(end):
            raise ValueError(f'`end` must be a valid date in ISO-8601 format. {end} is invalid')

        self.date_index: pd.DatetimeIndex = self._create_datetime_index(start, end)
        self.date_frame: pd.DataFrame = self.date_index.to_frame(name='full_date', index=True)
    ...
    def _add_names_and_numbers(self) -> None:
        ...
        self.date_frame.loc[:, 'day_name'] = \
            self.date_frame.loc[:, 'week_day_number'].apply(lambda n: calendar.day_name[n])

        self.date_frame.loc[:, 'day_name_abbr'] = \
            self.date_frame.loc[:, 'week_day_number'].apply(lambda n: calendar.day_abbr[n])

        self.date_frame.loc[:, 'month_name'] = \
            self.date_frame.loc[:, 'month_number'].apply(lambda n: calendar.month_name[n])

        self.date_frame.loc[:, 'month_name_abbr'] = \
            self.date_frame.loc[:, 'month_number'].apply(lambda n: calendar.month_abbr[n])
        ...
        self.date_frame.loc[:, 'week_number'] = \
            self.date_frame.loc[:, 'full_date'].apply(lambda d: 1 + int(d.strftime('%U')))
    ...
    def _add_last_weekdays_of_month(self) -> None:
        ...
        self.date_frame.loc[:, 'is_last_weekday_of_month'] = \
            self.date_frame.loc[:, 'full_date'].apply(lambda d: d in last_weekdays_per_month)

    def _add_accounting_periods(self) -> None:
        for i in range(1, 25):
            self.date_frame[f'date_plus_{i:02d}_months'] = self.date_frame['full_date'] + pd.DateOffset(months=i)
            self.date_frame[f'date_minus_{i:02d}_months'] = self.date_frame['full_date'] + pd.DateOffset(months=-i)
